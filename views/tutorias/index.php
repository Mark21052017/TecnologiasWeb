...
<main class="container">
    <div class="page-heading"><div><h1>Tutorias</h1><p>Solicitudes y sesiones de apoyo academico.</p></div><?php if ($role === 'estudiante'): ?><a class="button" href="<?= e(app_url('tutorias/create.php')) ?>">Solicitar tutoria</a><?php endif; ?></div>
    ...
    <div class="table-wrapper card"><table><thead><tr><th>Fecha</th><th>Materia</th><th>Estudiante</th><th>Tutor</th><th>Horario</th><th>Modalidad</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
        <?php foreach ($tutorias as $tutoria): ?><tr><td><?= e($tutoria['fecha']) ?></td><td><?= e($tutoria['nombre_materia']) ?></td><td><?= e($tutoria['estudiante']) ?></td><td><?= e($tutoria['tutor']) ?></td><td><?= e(substr($tutoria['hora_inicio'], 0, 5) . ' - ' . substr($tutoria['hora_fin'], 0, 5)) ?></td><td><?= e($tutoria['modalidad']) ?></td><td><span class="status status-<?= e($tutoria['estado']) ?>"><?= e($tutoria['estado']) ?></span></td><td class="actions">
            <?php if ($role === 'estudiante' && $tutoria['estado'] === 'pendiente'): ?><form method="post" action="<?= e(app_url('tutorias/status.php')) ?>"><input type="hidden" name="id" value="<?= (int) $tutoria['id_tutoria'] ?>"><input type="hidden" name="estado" value="cancelada"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><button class="link-button" type="submit">Cancelar</button></form><?php endif; ?>
        ...
        <?php if (!$tutorias): ?><tr><td colspan="8">No hay tutorias para mostrar.</td></tr><?php endif; ?>
    </tbody></table></div>
</main>
